...
class AudioFeatureExtractor:
    """Base contract for audio analysis implementations."""
    
    def extract(self, audio_path: str | Path) -> AudioFeatureSet:
        raise NotImplementedError


class LibrosaFeatureExtractor(AudioFeatureExtractor):
    """Lightweight extractor using standard DSP techniques via Librosa."""
    ...
    def extract(self, audio_path: str | Path) -> AudioFeatureSet:
        ...
        bpm = self._estimate_bpm(waveform, sample_rate)
        loudness = self._estimate_loudness(waveform)
        valence, energy = self._estimate_mood(waveform, sample_rate)
        ...
        embedding = self._build_audio_embedding(mel_spectrogram)
        has_vocals = self._estimate_vocals(mel_spectrogram)
    ...
    def _estimate_bpm(self, waveform: np.ndarray, sample_rate: int) -> float | None:
        try:
    ...
    def _estimate_loudness(self, waveform: np.ndarray) -> float:
        rms = float(np.mean(librosa.feature.rms(y=waveform)))
        return float(20 * np.log10(max(rms, 1e-8)))

    def _estimate_mood(self, waveform: np.ndarray, sample_rate: int) -> tuple[float, float]:
        rms = float(np.mean(librosa.feature.rms(y=waveform)))
    ...
    def _build_audio_embedding(self, mel_spectrogram: np.ndarray) -> list[float]:
    ...
    def _estimate_vocals(self, mel_spectrogram: np.ndarray) -> bool:
...
_default_extractor = LibrosaFeatureExtractor()


def extract_audio_features(audio_path: str | Path) -> AudioFeatureSet:
    """Legacy entry point delegating to the default extractor."""
    return _default_extractor.extract(audio_path)
