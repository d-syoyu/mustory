...
4. **コメント表示**
   - トラック詳細画面では、**物語・コメントをタブ or セグメント＋専用シートで表示**し、
     - 「物語」側：物語本文＋物語コメント一覧＋物語コメント投稿導線（モーダルシートでも可）
     - 「コメント」側：トラックコメント一覧＋トラックコメント投稿導線
   - タブ／セグメントはヘッダー直下に固定し、どのスクロール量でもワンタップで該当コメントの入力ビュー（タブ内 or シート）へ移動できる UI とする。
5. **リアクション**
   - 曲・物語・コメントへの「いいね」。
...
- **Track Detail**
  - プレイヤー（シークバーや波形風 UI）とヒーローカード（アートワーク、タイトル、アーティスト）。
  - いいね・シェア・コメント呼び出しボタンはヘッダー直下にまとめ、常に操作できるようにする。
  - 上部に「曲 / ストーリー」セグメントを配置し、以下を満たす：
    - **曲セグメント**
      - 再生コントロール＋トラック統計（再生数／いいね／コメント数）。
      - 「コメントを見る」アクションからトラックコメント用のボトムシートを開き、一覧＋投稿欄を表示する。
    - **ストーリーセグメント**
      - 物語のリード＋本文をページ内に表示。
      - コメントボタンから物語コメント用のボトムシートを開く。
      - 物語未作成時は作者向け CTA（物語作成ボタン）と閲覧者向けメッセージを表示。
  - 各ボトムシートはスクロール位置に関係なく入力欄を常に表示し、投稿後はリストを即時更新する。
